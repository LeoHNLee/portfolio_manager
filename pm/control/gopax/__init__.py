...
from pm.log import log, log_order, log_request, log_response
...
class Gopax(Controller):
    ...
    def request(self, method: str, path: str, body: Optional[dict] = None, recv_window: Optional[int] = None):
        ...
        if (
            ...
            or resp.json().get("errorMessage") is not None
        ):
            pass
        log_response(resp.status_code, method, self.HOST+path, resp.headers, resp.text)
        return resp
    ...
    def create_header(
        self, method: str, path: str, body: Optional[dict] = None, recv_window: Optional[int] = None
    ):
