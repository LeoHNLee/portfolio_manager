...
class SHI(Controller):
    ...
    def init(self):
        ...
        self["current_amt"] = self.apply(
            lambda x: self.calc_current_amt(x, tmp_df), axis=1
        )
        self["current_val"] = self.apply(
            lambda x: self.calc_current_val(x, tmp_df), axis=1
        )
    ...
    def get_flow(
        ...
        time.sleep(1)
        ui.ButtonControl(
            searchDepth=6, Name="예(Y)", AutomationId="CommandButton_6"
        ).Click()
    ...
    def bid(self, ticker: str, amt: int, cprice: int, bf_amt):
        ...
        try:
            ui.WindowControl(searchDepth=2, Name="(3754)미니주문(미국)").SetFocus()
            time.sleep(1)
            ui.EditControl(searchDepth=5, AutomationId="3810").SendKeys(
                ticker + "{Enter}"
            )
            time.sleep(1)
            ui.EditControl(searchDepth=5, AutomationId="3809").SendKeys(
                str(amt) + "{Enter}"
            )
            time.sleep(1)
    ...
    def ask(self, ticker: str, amt: int, cprice: int, bf_amt: int):
        ...
        try:
            ui.WindowControl(searchDepth=2, Name="(3651)주식주문(미국/홍콩/후강퉁/선강퉁)").SetFocus()
            time.sleep(1)
            ui.EditControl(searchDepth=7, AutomationId="3812").SendKeys(
                ticker + "{Enter}"
            )
            time.sleep(1)
            ui.EditControl(searchDepth=7, AutomationId="3811").SendKeys(
                str(amt) + "{Enter}"
            )
            time.sleep(1)
